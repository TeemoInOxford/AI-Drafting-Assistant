/**
 * Find all players with 2-5 total secondary position games
 * Including players with multiple secondary positions
 */
...
async function main() {
  ...
  for (const [playerId, stats] of playerPositionCount.entries()) {
    ...
    if (positionsPlayed.length > 1) {
      ...
      // 副位置总和在2-5场
      if (totalSecondary >= 2 && totalSecondary <= 5) {
        twoToFivePlayers.push({
          playerId,
  ...
  // 按选手名字排序
  twoToFivePlayers.sort((a, b) => a.playerName.localeCompare(b.playerName));

  console.log(`找到 ${twoToFivePlayers.length} 个副位置总和2-5场的选手\n`);
  console.log('='.repeat(120));
  console.log('副位置总和2-5场的选手详情\n');
  console.log('| # | 选手 | 战队 | 主位置(场次) | 副位置详情 | 副位置总和 |');
  console.log('|---|------|------|--------------|------------|------------|');

  for (let i = 0; i < twoToFivePlayers.length; i++) {
    const p = twoToFivePlayers[i];
    const mainStr = `${p.mainRole}(${p.mainCount})`;
    const secondaryStr = p.secondaryRoles.map(s => `${s.role}(${s.count})`).join(', ');

    console.log(`| ${String(i + 1).padStart(3)} | ${p.playerName.padEnd(15)} | ${p.teamName.slice(0, 25).padEnd(25)} | ${mainStr.padEnd(12)} | ${secondaryStr.padEnd(30)} | ${String(p.totalSecondary).padStart(10)} |`);
  }

  console.log('\n' + '='.repeat(120));
  console.log(`\n总计: ${twoToFivePlayers.length} 个选手\n`);
}
