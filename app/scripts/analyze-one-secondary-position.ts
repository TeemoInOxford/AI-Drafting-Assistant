/**
 * Find all players with exactly 1 secondary position game
 * Show detailed information about that specific game
 */
...
async function main() {
  ...
  // 找出副位置只有1场的选手
  const oneSecondaryPlayers = new Map<string, { mainRole: string; secondaryRole: string }>();
  ...
  for (const [playerId, stats] of playerPositionCount.entries()) {
    ...
    if (positionsPlayed.length === 2) {
      ...
      // 副位置只有1场
      if (stats.positions[secondaryRole] === 1) {
        oneSecondaryPlayers.set(playerId, { mainRole, secondaryRole });
      }
    }
  }

  console.log(`找到 ${oneSecondaryPlayers.size} 个副位置只有1场的选手\n`);

  // 第二遍：找出这些选手的那一场比赛详情
  const secondaryGames: SecondaryPositionGame[] = [];
  ...
  for (const file of files) {
    ...
    for (const game of series.games) {
      ...
      for (const team of game.teams) {
        ...
        for (const inf of inferred) {
          const playerInfo = oneSecondaryPlayers.get(inf.playerId);
  ...
  // 按选手名字排序
  secondaryGames.sort((a, b) => a.playerName.localeCompare(b.playerName));

  console.log('='.repeat(120));
  console.log('副位置只有1场的选手详情\n');
  console.log('| # | 选手 | 战队 | 主位置 | 副位置 | Series ID | Game | 日期 | 对手 | 英雄 | Neutral | Minion |');
  console.log('|---|------|------|--------|--------|-----------|------|------|------|------|---------|--------|');

  for (let i = 0; i < secondaryGames.length; i++) {
    const g = secondaryGames[i];
    console.log(`| ${String(i + 1).padStart(2)} | ${g.playerName.padEnd(12)} | ${g.teamName.slice(0, 20).padEnd(20)} | ${g.mainRole.padEnd(7)} | ${g.secondaryRole.padEnd(7)} | ${g.seriesId.padEnd(9)} | ${String(g.gameNumber).padStart(4)} | ${g.date} | ${g.opponentTeam.slice(0, 20).padEnd(20)} | ${g.champion.padEnd(12)} | ${String(g.neutralMinion).padStart(7)} | ${String(g.minion).padStart(6)} |`);
  }

  console.log('\n' + '='.repeat(120));
  console.log(`\n总计: ${secondaryGames.length} 场比赛`);
}
