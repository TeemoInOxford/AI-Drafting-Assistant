/**
 * Find all players with 6-10 total secondary position games
 */
...
async function main() {
  ...
  for (const [playerId, stats] of playerPositionCount.entries()) {
    ...
    if (positionsPlayed.length > 1) {
      ...
      if (totalSecondary >= 6 && totalSecondary <= 10) {
        sixToTenPlayers.push({
          playerId,
  ...
  sixToTenPlayers.sort((a, b) => a.playerName.localeCompare(b.playerName));

  console.log(`找到 ${sixToTenPlayers.length} 个副位置总和6-10场的选手\n`);
  console.log('='.repeat(130));
  console.log('副位置总和6-10场的选手详情\n');
  console.log('| # | 选手 | 战队 | 总场次 | 主位置(场次) | 副位置详情 | 副位置总和 |');
  console.log('|---|------|------|--------|--------------|------------|------------|');

  for (let i = 0; i < sixToTenPlayers.length; i++) {
    const p = sixToTenPlayers[i];
    const mainStr = `${p.mainRole}(${p.mainCount})`;
    const secondaryStr = p.secondaryRoles.map(s => `${s.role}(${s.count})`).join(', ');

    console.log(`| ${String(i + 1).padStart(3)} | ${p.playerName.padEnd(15)} | ${p.teamName.slice(0, 25).padEnd(25)} | ${String(p.totalGames).padStart(6)} | ${mainStr.padEnd(12)} | ${secondaryStr.padEnd(35)} | ${String(p.totalSecondary).padStart(10)} |`);
  }

  console.log('\n' + '='.repeat(130));
  console.log(`\n总计: ${sixToTenPlayers.length} 个选手\n`);
}
