...
def convert_match_based_hierarchy():
    """转换基于比赛清洗后的数据为 API 格式"""

    # 加载清洗后的数据
    with open(os.path.join(DATA_DIR, "lol_hierarchy_match_based.json"), "r", encoding="utf-8") as f:
        regions_array = json.load(f)

    # 加载统计数据
    with open(os.path.join(DATA_DIR, "lol_stats_match_based.json"), "r", encoding="utf-8") as f:
        stats = json.load(f)
    ...
    api_format = {
        ...
        "stats": {
            "totalPlayers": stats["totalPlayersWithMatches"],
...
def main():
    print("=" * 60)
    print("转换基于比赛清洗后的数据为 API 格式")
    print("=" * 60)

    api_format = convert_match_based_hierarchy()

    # 保存转换后的数据
    output_path = os.path.join(DATA_DIR, "lol_hierarchy_match_based_api.json")
    ...
    print(f"  - 比赛: {api_format['stats']['totalMatches']}")
    print(f"\n实际数据:")
    print(f"  - 战队对象: {len(api_format['teams'])}")
    print(f"  - 选手对象: {len(api_format['players'])}")
